...
const fplTableBody = document.querySelector("#fpl tbody");

// Function to update localStorage with new entries
    function updateStorage() {
        const entries = [];
        // Loop through table rows and get data
        const rows = fplTableBody.querySelectorAll("tr");
        rows.forEach(row => {
    ...
    // Update localStorage
    localStorage.setItem("fplEntries", JSON.stringify(entries));
}
...
function addRowOnTop() {
    const newRow = fplTableBody.insertRow(0); // Insert at index 0 to add at the top
    newRow.innerHTML = `
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
...
function updateDateAndDay() {
    const cfplentDate = new Date();
    const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
    const formattedDate = cfplentDate.toLocaleDateString('en-US', options);
...
// Load stored entries fplom localStorage when the page loads
function loadStoredEntries() {
    const storedEntries = localStorage.getItem("fplEntries");
    if (storedEntries) {
        const entries = JSON.parse(storedEntries);
        entries.forEach(entry => {
            const newRow = fplTableBody.insertRow();
            newRow.innerHTML = `
