...
const lcrlTableBody = document.querySelector("#lcrl tbody");

// Function to update localStorage with new entries
    function updateStorage() {
        const entries = [];
        // Loop through table rows and get data
        const rows = lcrlTableBody.querySelectorAll("tr");
        rows.forEach(row => {
    ...
    // Update localStorage
    localStorage.setItem("lcrlEntries", JSON.stringify(entries));
}
...
function addRowOnTop() {
    const newRow = lcrlTableBody.insertRow(0); // Insert at index 0 to add at the top
    newRow.innerHTML = `
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
...
function loadStoredEntries() {
    const storedEntries = localStorage.getItem("lcrlEntries");
    if (storedEntries) {
        const entries = JSON.parse(storedEntries);
        entries.forEach(entry => {
            const newRow = lcrlTableBody.insertRow();
            newRow.innerHTML = `
