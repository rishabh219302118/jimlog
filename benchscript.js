...
// Function to update localStorage with new entries
    function updateStorage() {
        ...
        const rows = benchTableBody.querySelectorAll("tr");
        rows.forEach(row => {
    ...
    // Update localStorage
    localStorage.setItem("benchEntries", JSON.stringify(entries));
}
...
function addRowOnTop() {
    const newRow = benchTableBody.insertRow(0); // Insert at index 0 to add at the top
    newRow.innerHTML = `
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
...
function loadStoredEntries() {
    const storedEntries = localStorage.getItem("benchEntries");
    if (storedEntries) {
        const entries = JSON.parse(storedEntries);
        entries.forEach(entry => {
            const newRow = benchTableBody.insertRow();
            newRow.innerHTML = `
