...
const rdfTableBody = document.querySelector("#rdf tbody");

// Function to update localStorage with new entries
    function updateStorage() {
        const entries = [];
        // Loop through table rows and get data
        const rows = rdfTableBody.querySelectorAll("tr");
        rows.forEach(row => {
    ...
    // Update localStorage
    localStorage.setItem("rdfEntries", JSON.stringify(entries));
}
...
function addRowOnTop() {
    const newRow = rdfTableBody.insertRow(0); // Insert at index 0 to add at the top
    newRow.innerHTML = `
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
...
// Load stored entries rdfom localStorage when the page loads
function loadStoredEntries() {
    const storedEntries = localStorage.getItem("rdfEntries");
    if (storedEntries) {
        const entries = JSON.parse(storedEntries);
        entries.forEach(entry => {
            const newRow = rdfTableBody.insertRow();
            newRow.innerHTML = `
