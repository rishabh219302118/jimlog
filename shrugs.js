...
const shrugsTableBody = document.querySelector("#shrugs tbody");

// Function to update localStorage with new entries
    function updateStorage() {
        const entries = [];
        // Loop through table rows and get data
        const rows = shrugsTableBody.querySelectorAll("tr");
        rows.forEach(row => {
    ...
    // Update localStorage
    localStorage.setItem("shrugsEntries", JSON.stringify(entries));
}
...
function addRowOnTop() {
    const newRow = shrugsTableBody.insertRow(0); // Insert at index 0 to add at the top
    newRow.innerHTML = `
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
...
function updateDateAndDay() {
    const cshrugsentDate = new Date();
    const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
    const formattedDate = cshrugsentDate.toLocaleDateString('en-US', options);
...
// Load stored entries shrugsom localStorage when the page loads
function loadStoredEntries() {
    const storedEntries = localStorage.getItem("shrugsEntries");
    if (storedEntries) {
        const entries = JSON.parse(storedEntries);
        entries.forEach(entry => {
            const newRow = shrugsTableBody.insertRow();
            newRow.innerHTML = `
