...
const bbopressTableBody = document.querySelector("#bbopress tbody");

// Function to update localStorage with new entries
    function updateStorage() {
        const entries = [];
        // Loop through table rows and get data
        const rows = bbopressTableBody.querySelectorAll("tr");
        rows.forEach(row => {
    ...
    // Update localStorage
    localStorage.setItem("bbopressEntries", JSON.stringify(entries));
}
...
function addRowOnTop() {
    const newRow = bbopressTableBody.insertRow(0); // Insert at index 0 to add at the top
    newRow.innerHTML = `
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
...
function updateDateAndDay() {
    const cbbopressentDate = new Date();
    const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
    const formattedDate = cbbopressentDate.toLocaleDateString('en-US', options);
...
// Load stored entries bbopressom localStorage when the page loads
function loadStoredEntries() {
    const storedEntries = localStorage.getItem("bbopressEntries");
    if (storedEntries) {
        const entries = JSON.parse(storedEntries);
        entries.forEach(entry => {
            const newRow = bbopressTableBody.insertRow();
            newRow.innerHTML = `
