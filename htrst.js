...
const htrstTableBody = document.querySelector("#htrst tbody");

// Function to update localStorage with new entries
    function updateStorage() {
        const entries = [];
        // Loop through table rows and get data
        const rows = htrstTableBody.querySelectorAll("tr");
        rows.forEach(row => {
    ...
    // Update localStorage
    localStorage.setItem("htrstEntries", JSON.stringify(entries));
}
...
function addRowOnTop() {
    const newRow = htrstTableBody.insertRow(0); // Insert at index 0 to add at the top
    newRow.innerHTML = `
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
...
function loadStoredEntries() {
    const storedEntries = localStorage.getItem("htrstEntries");
    if (storedEntries) {
        const entries = JSON.parse(storedEntries);
        entries.forEach(entry => {
            const newRow = htrstTableBody.insertRow();
            newRow.innerHTML = `
