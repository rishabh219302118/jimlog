...
const dbopressTableBody = document.querySelector("#dbopress tbody");

// Function to update localStorage with new entries
    function updateStorage() {
        const entries = [];
        // Loop through table rows and get data
        const rows = dbopressTableBody.querySelectorAll("tr");
        rows.forEach(row => {
    ...
    // Update localStorage
    localStorage.setItem("dbopressEntries", JSON.stringify(entries));
}
...
function addRowOnTop() {
    const newRow = dbopressTableBody.insertRow(0); // Insert at index 0 to add at the top
    newRow.innerHTML = `
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
...
function updateDateAndDay() {
    const cdbopressentDate = new Date();
    const options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' };
    const formattedDate = cdbopressentDate.toLocaleDateString('en-US', options);
...
// Load stored entries dbopressom localStorage when the page loads
function loadStoredEntries() {
    const storedEntries = localStorage.getItem("dbopressEntries");
    if (storedEntries) {
        const entries = JSON.parse(storedEntries);
        entries.forEach(entry => {
            const newRow = dbopressTableBody.insertRow();
            newRow.innerHTML = `
