...
const DeadTableBody = document.querySelector("#Dead tbody");

// Function to update localStorage with new entries
    function updateStorage() {
        const entries = [];
        // Loop through table rows and get data
        const rows = DeadTableBody.querySelectorAll("tr");
        rows.forEach(row => {
    ...
    // Update localStorage
    localStorage.setItem("DeadEntries", JSON.stringify(entries));
}
...
function addRowOnTop() {
    const newRow = DeadTableBody.insertRow(0); // Insert at index 0 to add at the top
    newRow.innerHTML = `
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
...
function loadStoredEntries() {
    const storedEntries = localStorage.getItem("DeadEntries");
    if (storedEntries) {
        const entries = JSON.parse(storedEntries);
        entries.forEach(entry => {
            const newRow = DeadTableBody.insertRow();
            newRow.innerHTML = `
