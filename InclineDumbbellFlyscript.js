...
const InclineDumbbellFlyTableBody = document.querySelector("#InclineDumbbellFly tbody");

// Function to update localStorage with new entries
    function updateStorage() {
        const entries = [];
        // Loop through table rows and get data
        const rows = InclineDumbbellFlyTableBody.querySelectorAll("tr");
        rows.forEach(row => {
    ...
    // Update localStorage
    localStorage.setItem("InclineDumbbellFlyEntries", JSON.stringify(entries));
}
...
function addRowOnTop() {
    const newRow = InclineDumbbellFlyTableBody.insertRow(0); // Insert at index 0 to add at the top
    newRow.innerHTML = `
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
        <td contenteditable="true"></td>
...
function loadStoredEntries() {
    const storedEntries = localStorage.getItem("InclineDumbbellFlyEntries");
    if (storedEntries) {
        const entries = JSON.parse(storedEntries);
        entries.forEach(entry => {
            const newRow = InclineDumbbellFlyTableBody.insertRow();
            newRow.innerHTML = `
